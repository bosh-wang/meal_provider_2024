...
def create_order(data):
    ...
    items = data.get(
        "items"
    )  # List of items, each item is a dict with item_id and quantity
    ...
    try:
        ...
        # Create new order
        cursor.execute(
            "INSERT INTO orders (user_id, restaurant_id, order_status) VALUES (%s, %s, %s) RETURNING order_id, order_date",
        ...
        for item in items:
            ...
            cursor.execute(
                "SELECT price FROM menus_items WHERE item_id = %s", (item_id,)
            )
            item_price = cursor.fetchone()["price"]

            cursor.execute(
                "INSERT INTO orders_items (order_id, item_id, quantity, price) VALUES (%s, %s, %s, %s)",
...
def change_order_status(data):
    ...
    try:
        ...
        cursor.execute(
            "UPDATE orders SET order_status = %s WHERE order_id = %s AND order_status = %s",
        ...
        if cursor.rowcount == 1:
            ...
            return make_response(
                jsonify({"message": "Order status changed successfully"}), 200
            )
        else:
            return make_response(
                jsonify({"error": "Invalid order ID or order status"}), 404
            )
